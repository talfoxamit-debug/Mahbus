...
global.document={createElement:()=>({width:0,height:0,getContext:mkCtx}),getElementById:()=>({style:{},classList:{add(){},remove(){}},getContext:mkCtx,addEventListener(){},textContent:'',offsetHeight:10,offsetWidth:10,clientWidth:960,querySelector:()=>null}),addEventListener(){}};
...
src+='\nglobal.hardSequence=hardSequence;global.legalMoves=legalMoves;global.setDL=function(){AI_DEADLINE=Date.now()+900;};';
eval(src);
function empty(){const a=[];for(let i=0;i<=24;i++)a.push({top:null,count:0,trapped:null});return a;}
let stuck=0, slow=0, max=0;
for(let trial=0; trial<3000; trial++){
  ...
  let b=0; const pts=[]; while(pts.length<10){const i=2+(Math.random()*18|0); if(!pts.includes(i))pts.push(i);}
  for(const i of pts){if(b>=15)break;const c=Math.min(15-b,1+(Math.random()*2|0));st.points[i]={top:'B',count:c,trapped:null};b+=c;}
  st.points[20]={top:'W',count:1,trapped:null}; st.points[21]={top:'W',count:1,trapped:null};
  setDL();
  const t0=Date.now(); const seq=hardSequence(st); const dt=Date.now()-t0;
  if(dt>max)max=dt; if(dt>1100)slow++;
  if(legalMoves(st,'B').length>0 && (!seq||seq.length===0)) stuck++;
}
console.log(`3000 hard turns: max=${max}ms, over-1100ms=${slow}, empty-with-legal-moves=${stuck}`);
